...
public class WebController {
  ...
  protected ModelAndView myFromClass1(
    @ModelAttribute("User") LoginGetterandSetters User,
    HttpSession session
  ) {
    ...
    } else return LoginFailedModel();
  }

  @RequestMapping(value = "login")
  protected ModelAndView login_page(HttpSession session) {
    if (session.getAttribute("loggedinUser") == null) {
      return new ModelAndView("mainPage").addObject(
        "company_details",
        getCompanyDetails()
      );
    }
  ...
  protected ModelAndView ForForm(
    @ModelAttribute("registerUser") GetterRegisterDetails reg
  ) {
  ...
  protected ModelAndView takingOrder(
    @ModelAttribute("ordPlaced") Ordergetter odr
  )
    ...
    if (!isOrderPlaced) {
      ...
      saveOrder_Impl.saveOrderAddress(
        odr.getAdd1(),
        odr.getAdd2(),
        getOrderID()
      );
    ...
    System.out.println(
      "the fistrOrder check : " + saveOrder_Impl.firstOrder(getUsername())
    );
    ...
    placed_Order
      .addObject("order" + "Cost", orderList.orderCost(getOrderID()))
  ...
  protected ModelAndView pricesPage(HttpSession session, Model model) {
    ...
    for (ItemPrices i : pricesList) System.out.println(
      i.getItem_name() + "" + i.getItem_cost()
    );
  ...
  public int getObject(Object obj)
    ...
    for (Field field : obj.getClass().getDeclaredFields()) {
      if (isItemCountValid(obj, field)) {
        int itm_cost = saveOrder_Impl.calc_ItemCost(
          field.getName().toString(),
          field.getInt(obj),
          getOrderID()
        );

        saveOrder_Impl.my_orderSave(
          getUsername(),
          getOrderID(),
          field.getName().toString(),
          field.getInt(obj),
  ...
  public boolean isItemCountValid(Object obj, Field field)
    ...
    if (field.getType().equals(int.class)) if (
      field.getInt(obj) != 0
    ) return true;
