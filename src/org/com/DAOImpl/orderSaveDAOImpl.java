...
public class OrderSaveDAOImpl implements SaveOrderDAO {
  ...
  public int calc_ItemCost(String Name, int itm_Number, String string) {
    ...
    // final String saveqry1 = "SELECT item_count FROM my_order WHERE cloth_code = ?
    // AND order_id = ?";
    jdbcTemplate.setDataSource(dataSource);
  ...
  // the query exerts some extra pressure on the database. It extends for multiple
  // yet same users!
  // can be optimised by some unique MySQL query or using a if else before
  // exhausting the user.
